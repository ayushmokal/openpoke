...
class ConversationLog:
    ...
    def record_reply(self, content: str) -> None:
        # Deduplicate: skip if this exact reply was the last poke_reply
        last_reply = self._get_last_reply()
        if content == last_reply:
            logger.debug("Skipping duplicate reply - same as last reply")
            return
        # Also skip if this exact content is in recent replies (within last 10)
        recent_replies = self._get_recent_replies(limit=10)
        if content in recent_replies:
            logger.debug("Skipping duplicate reply - already in recent history")
            return
    ...
    def _get_last_reply(self) -> Optional[str]:
        """Get the content of the last poke_reply entry."""
        last_reply: Optional[str] = None
        for tag, _, payload in self.iter_entries():
            if tag == "poke_reply":
                last_reply = payload
        return last_reply

    def _get_recent_replies(self, limit: int = 10) -> List[str]:
        """Get the content of recent poke_reply entries."""
        replies: List[str] = []
        for tag, _, payload in self.iter_entries():
            if tag == "poke_reply":
                replies.append(payload)
        return replies[-limit:] if replies else []
    ...
    def to_chat_messages(self) -> List[ChatMessage]:
        ...
        for tag, timestamp, payload in self.iter_entries():
            ...
            # Skip consecutive duplicate messages (same role and content)
            if role == last_role and payload == last_content:
                continue

            messages.append(
                ChatMessage(role=role, content=payload, timestamp=normalized_timestamp)
            )
            last_role = role
