"""Ultrahuman tool schemas and actions for the execution agent.

This module provides tools for:
- Battery/Device info: Ring battery status, device info, reset operations
- Health Metrics: Sleep, HR, HRV, steps, recovery, glucose data
- Shipping/Orders: Replacement requests, address updates, warranty info
"""
...
_SCHEMAS: List[Dict[str, Any]] = [
    _convert_to_openai_schema(schema) for schema in ULTRAHUMAN_TOOL_SCHEMAS
]


def get_schemas() -> List[Dict[str, Any]]:
    """Return Ultrahuman tool schemas in OpenAI/OpenRouter format."""
    return _SCHEMAS
...
def build_registry(agent_name: str) -> Dict[str, Callable[..., Any]]:
    """Return Ultrahuman tool callables bound to a specific agent."""
    registry: Dict[str, Callable[..., Any]] = {}

    for tool_name, tool_fn in ULTRAHUMAN_TOOL_FUNCTIONS.items():
        # Wrap with logging
