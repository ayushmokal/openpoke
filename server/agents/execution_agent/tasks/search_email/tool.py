...
from server.gemini_client import request_chat_completion
...
ERROR_GEMINI_NOT_CONFIGURED = "Gemini API key not configured. Set GEMINI_API_KEY."
...
def _validate_gemini_config() -> Tuple[Optional[str], Optional[str]]:
    """Validate Gemini configuration and return (api_key, model) or (None, error)."""
    settings = get_settings()
    api_key = settings.gemini_api_key
    if not api_key:
        return None, ERROR_GEMINI_NOT_CONFIGURED
...
async def task_email_search(search_query: str) -> Any:
    ...
    api_key, model_or_error = _validate_gemini_config()
    if not api_key:
        logger.error(f"[EMAIL_SEARCH] Gemini not configured: {model_or_error}")
